...
function drawRows(){
	for(var i = 0; i < number_rows; i +=1){
		...
		fill(critterArray[rowOccupants[i]].displayed_color, critterArray[rowOccupants[i]].natural_saturation, 80);
...
function determineFoodAvailable(){
	...
	food_available_to_distribute = food_available_to_distribute + global_food_to_allocate; // set this to zero after
}
...
function dinnerTime(){
	for(var i = 0; i < uniqueRowOccupants.length; i++){
		if (critterArray[uniqueRowOccupants[i]].am_alive==1){
			critterArray[uniqueRowOccupants[i]].eatDinner();
...
function fillEmptyCellFromNeighbor(cellToFill, neighborCritterID){
	...
	critterArray[parentCritter].size = critterArray[parentCritter].size++; // tell the creature it's bigger
	critterArray[parentCritter].rows_occupied.push(tempCellToFill); // hopefully, add this to positions
	rowOccupants[tempCellToFill] = parentCritter;
	print("expanded from neighbor to fill ", tempCellToFill, " parent was ", parentCritter);
}
...
function everybodyChangeColors(){
	for(var i = 0; i < uniqueRowOccupants.length; i++){
		critterArray[[uniqueRowOccupants[i]]].updateNeighborInfo();
		critterArray[[uniqueRowOccupants[i]]].updateColors();
	}
}
