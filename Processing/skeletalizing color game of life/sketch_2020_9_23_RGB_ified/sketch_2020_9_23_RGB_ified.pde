...
// okay so if we do this, it'll average them out, which puts it increasingly towards the blue/green in the middle
//    cur_hue = ((prev_hue + next_hue)*3 + (4*true_hue)) / 10;

// what if we add them all up, and then just... see how many laps it does?
    cur_hue = ((prev_hue + next_hue)*3 + (4*true_hue)) % 360;
