function Critter(position) {
  ...
  this.updateColors = function () {
    ...
    let temp_prev_neighbor_color =
      critterArray[this.prev_neighbor_location].displayed_color;
    let temp_next_neighbor_color =
      critterArray[this.next_neighbor_location].displayed_color;
    ...
    let change_towards_true = fancyCalc(
      this.displayed_color,
      this.natural_color
    );
    let change_towards_prev = fancyCalc(
      this.displayed_color,
      temp_prev_neighbor_color
    );
    let change_towards_next = fancyCalc(
      this.displayed_color,
      temp_next_neighbor_color
    );
    ...
    desired_change =
      change_towards_true + change_towards_next + change_towards_prev;
    ...
    if (
      desired_distance > 0 &&
      desired_distance * food_cost_to_move < this.food
    ) {
      // only change if it has enough energy to get there
